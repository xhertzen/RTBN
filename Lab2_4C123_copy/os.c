...
void(*thread1_ptr)(void) = NULL;
void(*thread2_ptr)(void) = NULL;
uint32_t thread1_period = 0;
uint32_t thread2_period = 0;
...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  void(*thread2)(void), uint32_t period2){
	...
	thread1_ptr = thread1;
  thread2_ptr = thread2;
  thread1_period = period1;
  thread2_period = period2; 		
...
//	if(Counter % 2 == 0)
//	{
		(*thread1_ptr)(); // invoke task 1
//	}
	
	if(Counter % 100 == 0)
	{
		(*thread2_ptr)(); // invoke task 2
	}
