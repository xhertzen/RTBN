...
// **similar to Lab 3. initialize priority field****
  tcbs[0].next = &tcbs[1]; tcbs[0].priority = p0; // 0 points to 1
  tcbs[1].next = &tcbs[2]; tcbs[1].priority = p1; // 1 points to 2
  tcbs[2].next = &tcbs[3]; tcbs[2].priority = p2; // 2 points to 3
  tcbs[3].next = &tcbs[4]; tcbs[3].priority = p3; // 3 points to 4
	tcbs[4].next = &tcbs[5]; tcbs[4].priority = p4; // 4 points to 5
  tcbs[5].next = &tcbs[6]; tcbs[5].priority = p5; // 5 points to 6
  tcbs[6].next = &tcbs[7]; tcbs[6].priority = p6; // 6 points to 7
  tcbs[7].next = &tcbs[0]; tcbs[7].priority = p7; // 7 points to 0										
...
// If there are multiple highest priority (not blocked, not sleeping) run these round robin
	...
	Counter++;
	uint32_t max_priority = 7;
	
	cur_tcb_pt = RunPt;
	cur_tcb_pt = cur_tcb_pt->next;
	
	while(cur_tcb_pt != RunPt)
	{
		 if(!cur_tcb_pt->blocked && !cur_tcb_pt->sleep)
		 {
			 if(cur_tcb_pt->priority < max_priority)
			 {
				 max_priority = cur_tcb_pt->priority;
				 max_priority_pt = cur_tcb_pt;
			 }
		 }
		 cur_tcb_pt = cur_tcb_pt->next;
	}
	RunPt = max_priority_pt;
